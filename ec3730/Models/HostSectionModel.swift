...
@available(iOS 15.0, *)
class MonapiSectionModel: HostSectionModel {
    convenience init() {
        self.init(Monapi.current, service: Monapi.lookupService)
        self.storeModel = StoreKitModel.monapi
    }

    override func query(url: URL? = nil, completion block: (() -> ())? = nil) {
        guard let url = url?.standardized else {
            block?()
            return
        }
        
        self.service.query(["uri": url]) { (error, response: MonapiThreat?) in
