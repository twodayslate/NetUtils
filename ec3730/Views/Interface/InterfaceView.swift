...
struct InterfaceView: View {
    ...
    @ViewBuilder private func proxyView(_ item: (key: String, title: String), info proxyInfo: [String: Any?]) -> some View {
        if let value = proxyInfo[item.key] as? String {
            CopyCellView<Any>(title: item.title, content: value)
        } else if let values = proxyInfo[item.key] as? [String] {
            CopyCellView<Any>(title: item.title, rows: values.map { CopyCellRow(content: $0) })
        } else if let value = proxyInfo[item.key] as? Int {
            CopyCellView<Any>(title: item.title, content: String(describing: value))
        } else if let value = proxyInfo[item.key] as? Bool {
            CopyCellView<Any>(title: item.title, content: value ? "Yes" : "No")
        } else if let value = proxyInfo[item.key] {
            CopyCellView<Any>(title: item.key, content: String(describing: value))
        }
    }

    @ViewBuilder func basicInfo() -> some View {
        CopyCellView<Any>(title: "Name", content: interface.name)
        if let address = interface.address {
            CopyCellView<Any>(title: "Address", content: address)
        }
        if let netmask = interface.netmask {
            CopyCellView<Any>(title: "Netmask", content: netmask)
        }
        if let broadcastAddress = interface.broadcastAddress {
            CopyCellView<Any>(title: "Broadcast Address", content: broadcastAddress)
        }
        CopyCellView<Any>(title: "Family", content: interface.family.toString())
        CopyCellView<Any>(title: "Loopback", content: interface.isLoopback ? "Yes" : "No")
        CopyCellView<Any>(title: "Runing", content: interface.isRunning ? "Yes" : "No")
        CopyCellView<Any>(title: "Up", content: interface.isUp ? "Yes" : "No")
        CopyCellView<Any>(title: "Supports Multicast", content: interface.supportsMulticast ? "Yes" : "No")
    }
    ...
    @ViewBuilder func wifiInfo() -> some View {
        if isCurrentWifiInterface, let interfaceInfo = model.wifiInterfaceInfo {
            if let SSID = interfaceInfo[kCNNetworkInfoKeySSID as String] as? String {
                CopyCellView<Any>(title: "SSID", content: SSID)
            }
            if let BSSID = interfaceInfo[kCNNetworkInfoKeyBSSID as String] as? String {
                CopyCellView<Any>(title: "BSSID", content: BSSID)
            }
        }
    }
